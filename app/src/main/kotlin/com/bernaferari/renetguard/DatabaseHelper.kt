...
class DatabaseHelper private constructor(context: Context) :
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        ...
        try {
            ...
            if (version == DB_VERSION) {
                ...
                throw IllegalArgumentException("$DB_NAME upgraded to $version but required $DB_VERSION")
            }
        } catch (ex: Throwable) {
            Log.e(TAG, ex.toString() + "\n" + Log.getStackTraceString(ex))
        } finally {
    ...
    fun clearLog(uid: Int) {
        ...
        try {
            ...
            try {
                if (uid < 0) {
                    db.delete("log", null, arrayOf())
                } else {
                    db.delete("log", "uid = ?", arrayOf(uid.toString()))
                }
        ...
        try {
            ...
            try {
                ...
                if (rows == 0) {
                    ...
                    if (db.insert("access", null, cv) == -1L) {
    ...
    fun updateUsage(usage: Usage, dname: String?) {
        ...
        try {
            ...
            try {
                ...
                db.query(
                    "access",
                    arrayOf("sent", "received", "connections"),
                    selection,
                    selectionArgs,
                    null,
                    null,
    ...
    fun setAccess(id: Long, block: Int) {
        ...
        try {
            ...
            try {
                ...
                if (db.update("access", cv, "ID = ?", arrayOf(id.toString())) != 1) {
    ...
    fun clearAccess(uid: Int, keeprules: Boolean) {
        ...
        try {
            ...
            try {
                ...
                if (keeprules) {
                    db.delete("access", "uid = ? AND block < 0", arrayOf(uid.toString()))
                } else {
                    db.delete("access", "uid = ?", arrayOf(uid.toString()))
                }
    ...
    fun getAccess(uid: Int): Cursor {
        ...
        try {
            ...
            query +=
                ", (SELECT COUNT(DISTINCT d.qname) FROM dns d WHERE d.resource IN (SELECT d1.resource FROM dns d1 WHERE d1.qname = a.daddr)) count"
            query += " FROM access a"
            query += " WHERE a.uid = ?"
    ...
    fun getHostCount(uid: Int, usecache: Boolean): Long {
        ...
        try {
            ...
            val hosts =
                db
                    .compileStatement("SELECT COUNT(*) FROM access WHERE block >= 0 AND uid =$uid")
                    .simpleQueryForLong()
            synchronized(mapUidHosts) {
            ...
            try {
                ...
                val rows =
                    db.update(
                        "dns",
                        cv,
                        "qname = ? AND aname = ? AND resource = ?",
                        arrayOf(rr.QName, rr.AName, rr.Resource),
                    )
        ...
        try {
            ...
            try {
                db.delete(
                    "forward",
                    "protocol = ? AND dport = ?",
                    arrayOf(protocol.toString(), dport.toString()),
                )
